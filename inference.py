...
def dota_evaluate(model,
    ...
    im = cv2.cvtColor(cv2.imread(img_path, cv2.IMREAD_COLOR), cv2.COLOR_BGR2RGB)
    dets = im_detect(model, im, target_sizes=target_size, conf = conf)
    nt += len(dets)
    res = sort_corners(rbox_2_quad(dets[:, 2:]))
    for k in range(dets.shape[0]):
        inference_ls.append('{:.0f} {:.0f} {:.0f} {:.0f} {:.0f} {:.0f} {:.0f} {:.0f} {} {} {:.2f}\n'.format(
            res[k, 0], res[k, 1], res[k, 2], res[k, 3],
            res[k, 4], res[k, 5], res[k, 6], res[k, 7],
...
def evaluate(target_size,
    ...
    if model is None:
        ...
        if weight.endswith('.pth'):
            ...
            for k in state_dict:
                state_dict_[k.partition('module.')[2]] = state_dict[k]
            # load model
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weight', type=str, default='/media/qisens/2tb1/python_projects/training_pr/Cascade-Rotated-RetinaNet/weights/best.pth')
    ...
    # parser.add_argument('--dataset', nargs='?', type=str, default='ICDAR15')
    # parser.add_argument('--test_path', nargs='?', type=str, default='ICDAR15/test')
